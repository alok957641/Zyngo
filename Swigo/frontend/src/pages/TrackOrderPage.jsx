...
const TrackOrderPage = () => {
  ...
  if (!currentOrder) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-white">
        ...
        <button onClick={() => navigate(-1)} className="mt-4 text-orange-500">
          Go Back
        </button>
      </div>
    );
  ...
  const userPos = [
    currentOrder?.delevryAddress?.latitude || 0,
    currentOrder?.delevryAddress?.longitude || 0,
  ];
  const shopPos = activeShopOrder?.shop?.location?.coordinates
    ? [
        activeShopOrder.shop.location.coordinates[1],
        activeShopOrder.shop.location.coordinates[0],
      ]
    : null;
    ...
    <div className="flex flex-col h-screen bg-white overflow-hidden font-sans">
      <header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-4 z-[2000] shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[10px] font-black uppercase tracking-[2px] text-gray-400">
      ...
      <main className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
        <div className="w-full h-[40vh] md:h-full md:flex-1 relative order-1 md:order-2 bg-gray-100">
          ...
          >
            ...
            <Marker position={userPos} icon={homeIcon}>
              <Popup>
                <b>Your Delivery Location</b>
              </Popup>
            </Marker>
            <Marker position={dboyPos} icon={scooterIcon}>
              <Popup>
                <span className="font-bold">{dbName} is on the way!</span>
              </Popup>
            </Marker>
            <Polyline
              positions={
                shopPos ? [shopPos, dboyPos, userPos] : [dboyPos, userPos]
              }
              color="#f97316"
              weight={3}
              dashArray="5, 10"
        ...
        <aside className="w-full md:w-[380px] h-[60vh] md:h-full bg-white order-2 md:order-1 z-50 shadow-[0_-15px_30px_rgba(0,0,0,0.1)] overflow-y-auto border-t md:border-t-0 md:border-r border-gray-100">
          <div className="p-6 space-y-6">
            ...
            <div
              className={`p-5 rounded-3xl border-2 flex items-center justify-between shadow-sm ${
            ...
            <div className="flex items-center gap-4 border-b border-gray-50 pb-5">
              ...
              <div>
                <h2 className="text-lg font-black text-gray-900 tracking-tighter uppercase leading-none">
                  {activeShopOrder?.shop?.name}
                </h2>
                <p className="text-[10px] text-orange-500 font-black uppercase mt-1 tracking-[2px] italic animate-pulse">
                  {activeShopOrder?.status}
                </p>
              </div>
            </div>
            ...
            {deliveryBoy ? (
              <motion.div
                ...
                className="bg-slate-900 p-5 rounded-[2.5rem] text-white shadow-xl shadow-slate-200"
              >
                ...
                <a
                  ...
                  className="w-full bg-white text-slate-900 py-4 rounded-2xl flex items-center justify-center gap-3 text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all shadow-md"
                >
