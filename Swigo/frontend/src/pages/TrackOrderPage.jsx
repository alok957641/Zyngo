...
// 🛰️ Ye Map ko focus mein rakhega jab rider move karega
function MapUpdater({ center }) {
    const map = useMap();
    useEffect(() => {
        if (center && center[0] && center[1]) {
            // Smoothly move karne ke liye flyTo use kiya hai
...
const TrackOrderPage = () => {
    ...
    const fetchOrder = async (isInitial = false) => {
        try {
            const { data } = await axios.get(`import.meta.env.VITE_API_URL/api/order/get-order-by-id/${orderId}`, { withCredentials: true });
            const orderData = data.order || data;
            if (orderData) {
                setCurrentOrder(orderData);
                console.log("🏎️ Rider Position Synced");
            }
    ...
    // 🔥 TERA REQUESTED SET-INTERVAL
    useEffect(() => {
        // 1. Pehli baar turant fetch karo
        fetchOrder(true);

        // 2. Har 3 second mein background mein update hota rahega
        const pollInterval = setInterval(() => {
            fetchOrder(false);
        }, 3000); 

        // 3. Cleanup: Page se bahar jate hi polling band
    ...
    const shopPos = activeShopOrder?.shop?.location?.coordinates ? [activeShopOrder.shop.location.coordinates[1], activeShopOrder.shop.location.coordinates[0]] : null;
    
    // 💡 Rider ki live location agar backend se aa rahi hai toh wo lo
        ...
        <div className="flex flex-col h-screen bg-white overflow-hidden font-sans">
            <header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-4 z-[2000] shadow-sm">
                <div className="flex items-center gap-3">
                    <motion.button whileTap={{ scale: 0.8 }} onClick={() => navigate(-1)} 
            ...
            <main className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
                <div className="w-full h-[40vh] md:h-full md:flex-1 relative order-1 md:order-2 bg-gray-100">
                    <MapContainer center={dboyPos} zoom={15} style={{ height: '100%', width: '100%', zIndex: 1 }} zoomControl={false}>
                        ...
                        <Polyline positions={shopPos ? [shopPos, dboyPos, userPos] : [dboyPos, userPos]} color="#f97316" weight={3} dashArray="5, 10" opacity={0.6} />
                        
                        {/* 🛰️ Ye logic map ko update karta rahega */}
                        <MapUpdater center={dboyPos} />
                    </MapContainer>
                </div>

                <aside className="w-full md:w-[380px] h-[60vh] md:h-full bg-white order-2 md:order-1 z-50 shadow-[0_-15px_30px_rgba(0,0,0,0.1)] overflow-y-auto border-t md:border-t-0 md:border-r border-gray-100 no-scrollbar">
                    <div className="p-6 space-y-6">
                            ...
                            <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                                {currentOrder?.shopOrders?.map((so, idx) => (
                                    <button 
                                        ...
                                        className={`flex-shrink-0 px-4 py-3 rounded-2xl border-2 transition-all flex items-center gap-2 ${activeShopIndex === idx ? 'border-orange-500 bg-orange-50 text-orange-600 shadow-md shadow-orange-100' : 'border-gray-100 text-gray-400 bg-white'}`}
                                    >
                                        <Store size={14} />
                        ...
                        <div className={`p-5 rounded-3xl border-2 flex items-center justify-between shadow-sm ${currentOrder?.paymentMethod === 'cod' ? 'bg-orange-50 border-orange-200' : 'bg-green-50 border-green-200'}`}>
                            <div className="flex items-center gap-3">
                                <div className={`p-2.5 rounded-2xl ${currentOrder?.paymentMethod === 'cod' ? 'bg-orange-500 text-white shadow-lg shadow-orange-200' : 'bg-green-600 text-white shadow-lg shadow-green-100'}`}>
                                ...
                                <div>
                                    ...
                                    <h3 className={`text-sm font-black uppercase tracking-tight ${currentOrder?.paymentMethod === 'cod' ? 'text-orange-600' : 'text-green-700'}`}>
