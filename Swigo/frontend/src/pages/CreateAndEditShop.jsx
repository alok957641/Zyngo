...
function CreateAndEditShop() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center py-8 px-4 font-sans">
      {/* ... (Baki ka UI code wahi hai jo tumne diya tha, maine copy paste kar diya hai) ... */}
      <div className="max-w-2xl w-full mb-6">
        <button onClick={handleBackClick} disabled={isGoingBack || isSubmitting} className="group flex items-center gap-3 text-slate-500 hover:text-orange-600 font-semibold transition-all w-fit">
          <div className="p-2.5 bg-white rounded-full shadow-sm border border-slate-200">{isGoingBack ? <FiLoader className="animate-spin" /> : <FiArrowLeft />}</div>
          <span>Back</span>
        </button>
      </div>
      ...
      <div className="max-w-2xl w-full bg-white shadow-xl rounded-[2.5rem] overflow-hidden border border-slate-100">
        <div className="bg-gradient-to-br from-orange-50 to-red-50 p-10 text-center relative">
          <div className="bg-white w-16 h-16 rounded-2xl text-orange-500 mb-4 shadow-sm border border-orange-100 mx-auto flex items-center justify-center"><FaStore className="text-3xl" /></div>
          <h2 className="text-3xl font-black text-slate-800 uppercase tracking-tight">{myShopData ? "Edit Your Shop" : "Start New Shop"}</h2>
        </div>
        ...
        <form onSubmit={handlesubmit} className="p-8 space-y-6">
          ...
          <div className="space-y-2">
             ...
             <div className="relative w-full h-44 bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl overflow-hidden group cursor-pointer">
                ...
                {frontendimg ? <img className="w-full h-full object-cover" src={frontendimg} alt="Preview" /> : <div className="h-full flex flex-col items-center justify-center text-slate-400"><FiUploadCloud className="text-3xl mb-2" /> <span className="text-xs font-bold">Tap to upload</span></div>}
             </div>
          </div>

          <div className="space-y-3">
             <label className="text-slate-700 font-bold text-sm ml-1 uppercase tracking-widest flex items-center gap-2"><FiMapPin className="text-orange-500" /> Mark Shop on Map *</label>
             <div className="h-[250px] w-full rounded-2xl overflow-hidden border-2 border-slate-100 shadow-inner z-0 relative">
                ...
                <div className="absolute bottom-2 left-2 right-2 bg-white/90 backdrop-blur-sm p-2 rounded-xl text-[10px] font-bold text-slate-600 z-[1000] border border-white">📍 {lat.toFixed(4)}, {lon.toFixed(4)}</div>
             </div>
          </div>
          ...
          <button type="submit" disabled={isSubmitting} className="w-full bg-slate-900 text-white font-black py-5 rounded-2xl shadow-lg hover:bg-orange-600 transition-all active:scale-95 flex items-center justify-center gap-3">
